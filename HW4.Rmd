...
for (s in 1:S) {
  lambda = exp(X.q %*% beta.q@coef[s,]) 
  Y.q[, s] = rnegbin(nrow(College.train), mu = lambda, theta = lambda/(sqrt(disperse.qpoi) - 1))
  rmse.rep.qpoi[s] = rmse(predict(Apps.qpoi), Y.q[, s])
}
