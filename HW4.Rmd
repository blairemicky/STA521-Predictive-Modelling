...
From the summaries given above, variables in both train set and test set have similar ranges and mean. One thing we need pay attention to is that the ratio of public school vs private school in training set is 0.43 while the ratio in test set is only 0.24. This may introduce bias since private school and public school have many difference.
...
GGPLOT.13.2<-GGPLOT.13+geom_vline(xintercept = rmse.qpoi.test, col="cornflowerblue")
...
#compute p-value
pval<-sum(RMSE.rep.qp>rmse.qpoi.train)/nsim
print(pval)
...
```
ANSWER: After adding in the transformations that we used in our linear model, the model improved a bit. Though estimated overdispersion is 1.05, the p-value is now at .17, indicating that we cannot reject this model. The residual plot and scale location plots resembles a uniform distribution- though the points are concentrated- and the only real cause for concern is some large residuals with the largest being around 8.
...
hist.nb <-ggplot(data= rmse.rep.nb, aes(x=rmse.rep.nb)) + geom_histogram(fill = "slateblue") + geom_vline(xintercept = rmse.train.nb, color = "red") + geom_vline(xintercept = rmse.test.nb, col="cornflowerblue")
print(hist.nb)

pval <-sum(rmse.rep.nb > rmse.test.nb)/S
print(pval)
```
ANSWER: From histogram, it is clear that the Negative Binomial helps to explain more of the variation in the data than the Poisson or Quasi-Poisson, as the RMSE on the test set is closer to the mean of the simulations than those of the former. However, the p-value is still 0, indicating that we should reject this model as well. 
...
qp.sim = sim(model, n.sims = 1000)
# Generate the design matrix 
X.q = model.matrix(model, data = College.test[,-17])
# Allocate the output 
...
pi.qpoi.2 = t(apply(Y, 1, function(x) {quantile(x, c((1 - .95)/2,.5 + .95/2))}))
...
coverage(apps.lm0,College.test[,-17])
coverage(app_best,College.test[,-17])
coverage(Apps.poi,College.test[,-17])
coverage(Apps.qpoi.2,College.test[,-17])
coverage(Apps.nb,College.test[,-17])
```
```{r}
