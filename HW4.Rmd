...
#plot the histogram 
GGPLOT.0 = ggplot(data= RMSE.2, aes(x=RMSE.2))+
             geom_histogram() + 
...
b.new<-predict(app_lm_best, newdata=College.test)
...
par(mfrow=c(1,2))
plot(x = a.new[-ind.a], y = a.new[-ind.a]-College.test$Apps[-ind.a],
     ylab = "residual", 
     xlab = "fitted value",
     main = "residual vs fitted value(main effect)")
plot(x = exp(b.new[-ind.b]), y = exp(b.new[-ind.b])-College.test$Apps[-ind.b],
     ylab = "residual", 
     xlab = "fitted value",
     main = "residual vs fitted value(best)")
...
```{r}
Apps.poi = glm(Apps ~ . -Apps -college, family = poisson(link = "log"), data = College.train)
summary(Apps.poi)
plot(Apps.poi)
```
...
rmse.rep.poi = rep(0, S)

for (s in 1:S) {
  Y[, s] = exp(X %*% beta@coef[s,]) + rnorm(1, 0, beta@sigma[s])
  rmse.rep.poi[s] = rmse(predict(Apps.poi), Y[, s] )
}

rmse.df = data.frame(rsme.train.poi = rmse.train.poi, rmse.rep.poi = rmse.rep.poi)
rmse.df
...
print(pval)
```
From the histogram, it is clear that the RMSE from the training data is much larger than those generated by simulations of the data set. This suggests that the model is not adequate, as it underestimates the true MSE.
...
```{r}
rsme.test = rmse(College.test$Apps, predict(Apps.poi))
hist.poi = hist.poi + geom_vline(xintercept = rsme.test, colour = "royalblue")
print(hist.poi)
```
Compared to the training RMSE, the test RMSE shows that the model is far worse than we previously would have imagined, as most of the simulations do not even show up in the grid. This may suggest overfitting or that our model is too flexible.
...
Apps.poi.trans <- glm(Apps ~ . -Apps - P.Undergrad - college, family = quasipoisson(link = "log"), data = College.test)
