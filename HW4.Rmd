...
GGPLOT.0<-ggplot(data= RMSE.2, aes(x=RMSE.2))+
  geom_histogram() + 
  geom_vline(xintercept = RMSE.1, color="red")
plot(GGPLOT.0)
#compute p-value
...
app_lm_t2<-lm(log(Apps) ~ (Private + log(Top10perc) + log(Top25perc) +       
                           log(F.Undergrad) +log(P.Undergrad) +  Outstate + 
                           (Room.Board) + log(Books) + log(Personal) +         
                           log(PhD)+Terminal+log(S.F.Ratio)+ perc.alumni +    
                           log(Expend) + log(Grad.Rate)+ Elite)^2, 
                           data=College.train)
...
set.seed(1)
app_lm_best = lm(formula = log(Apps) ~ Private + log(Top10perc) + log(Top25perc) + 
    ...
    log(Expend):log(Grad.Rate), data = College.train)
nsim=1000
rmse = function(y, ypred){
  rmse=sqrt(mean((y-ypred)^2))
  return(rmse)
...
summary(Apps.qpoi)
plot(Apps.qpoi)
>>>>>>> c70ff7cd1e6e31ea4fc05cd1929d05e4340d85f9
```
